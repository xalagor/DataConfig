...
DC_TEST("DataConfig.Extra.SerDe.NestedVec2")
{
...

	{
		...
		[](FDcDeserializeContext& Ctx) {
			Ctx.Deserializer->AddStructHandler(TBaseStructure<FVector2D>::Get(), FDcDeserializeDelegate::CreateStatic(HandlerVector2DDeserialize));
		}));
...

	{
		...
		[](FDcSerializeContext& Ctx) {
			Ctx.Serializer->AddStructHandler(TBaseStructure<FVector2D>::Get(), FDcSerializeDelegate::CreateStatic(HandlerVector2DSerialize));
		}));
		Writer.Sb << TCHAR('\n');
...
DC_TEST("DataConfig.Extra.SerDe.NestedGrid2D")
{
...

	{
		...
		[](FDcDeserializeContext& Ctx) {
			Ctx.Deserializer->AddStructHandler(TBaseStructure<FDcGrid2D>::Get(), FDcDeserializeDelegate::CreateStatic(HandlerGrid2DDeserialize));
		}));
...

	{
		...
		[](FDcSerializeContext& Ctx) {
			Ctx.Serializer->AddStructHandler(TBaseStructure<FDcGrid2D>::Get(), FDcSerializeDelegate::CreateStatic(HandlerGrid2DSerialize));
		}));
		Writer.Sb << TCHAR('\n');
